...
- [Cột đo kinh lạc là NUMERIC(6,2), không phải INT](facts/meridian-measurements-numeric-khong-int.md) — 24 cột kinh lạc trong meridian_measurements lưu số thực (vd 33.2, 32.3 mV), phải NUMERIC(6,2); đã fix + backfill 9.499 bản ghi trên production Aiven.
- [admins.id là UUID, patients.id là INT](facts/admins-uuid-patients-int-khac-kieu-id.md) — Hai bảng khoá chính khác kiểu: admins.id UUID, patients.id INT - gây lỗi khi viết SQL audit log nối giữa hai bảng, phải kiểm kiểu trước khi JOIN/FK.
- [Thuật toán confidence score cho rule engine BenhDongYExcel](facts/confidence-score-benh-dong-y-excel.md) — diagnose() giờ trả thêm score 0-100 = % mệnh đề CMP khớp/tổng mệnh đề trong logicExpression, level high(>=80%)/medium(>=50%)/low(<50%), bệnh khớp cao xếp đầu danh sách.
- [Chạy migration SQL thẳng từ máy local lên Aiven production, không qua VPS](facts/migration-sql-chay-truc-tiep-local-len-aiven.md) — psql cài qua brew (libpq, keg-only) rồi nối thẳng `$DATABASE_URL?sslmode=require` chạy migration lên Aiven - không cần pull code lên VPS hay tách CA cert vì app đã set rejectUnauthorized:false.
