...
const deletingItem = ref<KinhMachRow | null>(null)
...
const filteredList = computed(() => {
  ...
  return dataList.value.filter((row) => {
    const hay = [row.ten_kinh_mach, row.ten_viet_tat, row.ky_hieu_quoc_te, row.ngu_hanh]
      .filter(Boolean)
      .join(' ')
      .toLowerCase()
...
function openEditModal(row: KinhMachRow) {
  editingId.value = row.idKinhMach
  form.value = {
    ten_kinh_mach: row.ten_kinh_mach ?? '',
...
async function handleSubmit() {
  ...
  formError.value = null
  const f = form.value
  if (!f.ten_kinh_mach.trim()) {
    formError.value = 'Vui lòng nhập tên kinh mạch'
    return
  }
  const payload: Record<string, unknown> = {
    ten_kinh_mach: f.ten_kinh_mach.trim(),
    ten_viet_tat: f.ten_viet_tat.trim() || null,
    ky_hieu_quoc_te: f.ky_hieu_quoc_te.trim() || null,
    ngu_hanh: f.ngu_hanh.trim() || null,
  ...
  try {
    if (editingId.value != null) {
      await api.put(`/kinh-mach/${editingId.value}`, payload)
    } else {
      await api.post('/kinh-mach', payload)
    }
    await fetchData()
    closeModal()
  } catch (err: any) {
...
function confirmDelete(row: KinhMachRow) {
...
async function handleDelete() {
  ...
  isSubmitting.value = true
  try {
    await api.delete(`/kinh-mach/${deletingItem.value.idKinhMach}`)
  ...
  <div class="management-page">
    <div class="page-header">
      <div class="header-content">
        <h1 class="page-title">Quản Lý Kinh Mạch</h1>
        <p class="page-subtitle">Danh mục 12 kinh chính và các kinh mạch dùng trong chẩn đoán / điều trị</p>
      </div>
      <button type="button" class="btn-primary" @click="openCreateModal">+ Thêm kinh mạch</button>
    </div>
      ...
      <div class="toolbar">
        <label class="search-wrap">
          ...
          <input
            ...
            type="search"
            class="search-input"
            placeholder="Tìm theo tên, viết tắt, ký hiệu, ngũ hành..."
            autocomplete="off"
          />
        </label>
          ...
          <table class="data-table">
            <thead>
              <tr>
                <th width="64">ID</th>
                <th>Tên kinh mạch</th>
                <th width="120">Viết tắt</th>
                <th width="140">Ký hiệu quốc tế</th>
                <th width="120">Ngũ hành</th>
                <th width="100">Số huyệt</th>
            ...
            <tbody>
              ...
              <tr v-for="item in pagedList" :key="item.idKinhMach">
                <td class="font-bold cell-id">#{{ item.idKinhMach }}</td>
                <td class="text-brown-900 font-bold">{{ item.ten_kinh_mach || '—' }}</td>
                <td>
                  <span v-if="item.ten_viet_tat" class="chip chip-abbrev">{{ item.ten_viet_tat }}</span>
                  <span v-else class="muted">—</span>
                </td>
                <td>
                  <span v-if="item.ky_hieu_quoc_te" class="chip chip-intl">{{ item.ky_hieu_quoc_te }}</span>
                  <span v-else class="muted">—</span>
                </td>
                <td>
                  <span v-if="item.ngu_hanh" class="chip chip-nguhanh">{{ item.ngu_hanh }}</span>
                  <span v-else class="muted">—</span>
                </td>
                <td>{{ item.tong_so_huyet ?? '—' }}</td>
                <td class="text-right">
    ...
    <div v-if="showModal" class="modal-overlay" @click.self="closeModal">
      <div class="modal" @click.stop>
        <div class="modal-header">
          <h3>{{ editingId != null ? 'Sửa kinh mạch' : 'Thêm kinh mạch' }}</h3>
        ...
        <form class="modal-body" @submit.prevent="handleSubmit">
          ...
          <div class="form-grid">
            <label class="field field--full">
              <span>Tên kinh mạch <abbr title="bắt buộc">*</abbr></span>
              <input v-model="form.ten_kinh_mach" class="input" placeholder="vd. Thủ thái âm Phế kinh" maxlength="255" />
            </label>
            <label class="field">
              <span>Viết tắt</span>
              <input v-model="form.ten_viet_tat" class="input" placeholder="vd. Phế" maxlength="50" />
            </label>
            <label class="field">
              <span>Ký hiệu quốc tế</span>
              <input v-model="form.ky_hieu_quoc_te" class="input" placeholder="vd. LU" maxlength="50" />
            </label>
            <div class="field">
              <span class="field-label">Ngũ hành</span>
              <div class="chip-picker">
                <button
                  v-for="opt in NGU_HANH_OPTIONS"
                  :key="opt"
                  type="button"
                  class="chip-toggle"
                  :class="{ active: form.ngu_hanh === opt }"
                  @click="form.ngu_hanh = form.ngu_hanh === opt ? '' : opt"
                >
                  {{ opt }}
                </button>
              </div>
            ...
            <label class="field">
              <span>Tổng số huyệt</span>
              <input
                v-model.number="form.tong_so_huyet"
                type="number"
                min="0"
                class="input"
                placeholder="vd. 11"
    ...
    <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="showDeleteConfirm = false">
      <div class="modal modal--sm" @click.stop>
        ...
        <div class="modal-body">
          <p>
            Xóa kinh mạch <strong>{{ deletingItem?.ten_kinh_mach || `#${deletingItem?.idKinhMach}` }}</strong>?
            Thao tác không hoàn tác, có thể kéo theo xóa các huyệt vị liên kết.
          </p>
        </div>
        <div class="modal-footer">
...
.chip-abbrev { background: var(--success-bg); color: var(--success-fg); border-color: var(--success-border); }
.chip-intl { background: var(--info-bg); color: var(--info-fg); border-color: var(--info-border); font-family: ui-monospace, monospace; }
.chip-nguhanh { background: var(--warning-bg); color: var(--warning-fg); border-color: var(--warning-border); }
